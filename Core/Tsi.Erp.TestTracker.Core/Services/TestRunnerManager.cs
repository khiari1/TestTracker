...
//            {
//                var attributre = type.GetCustomAttributes(false);

//                var attributeType = attributre?
//                    .FirstOrDefault(a => a.GetType().Name == TestClassAttribute)?
//                    .GetType();
...
//                    .Where(a => a.GetType().Name == TestMethodAttribute || a.GetType().Name == TearDownAttribute)
//                    .FirstOrDefault();
...
//                    return new TestResult(TestState.Skipped, message, "", new TimeSpan(0), "", null);
//                }
...
//                    return new TestResult(TestState.Skipped, message, "", new TimeSpan(0), classType.FullName, null);
//                }
...
//                            ParamTypeCssClass = "textconsolas text-100",
//                            ParamNameCssClass = "textconsolas text-400",
//                            TypeCssClass = "textconsolas text-100",
//                            LineCssClass = "textconsolas text-100",
...
//                {
//                    return new TestResult(TestState.Skipped, result?.Message, stackTrace, timespan, Area, exception);
//                }
//                else if (status == TestStatus.Passed ||
//                    status == TestStatus.Inconclusive)
//                {

//                    return new TestResult(result?.Status == AssertionStatus.Warning ? TestState.Warning : TestState.Success,
//                        result?.Message ?? $"Test succeded with {status.ToString()}",
//                        stackTrace, timespan, Area, exception);
//                }
//                else if (status == TestStatus.Warning)
//                {
//                    return new TestResult(TestState.Warning, result?.Message, stackTrace, timespan, Area, exception);
//                }
//                else if (status == TestStatus.Failed)
//                {
//                    return new TestResult(result?.Status == AssertionStatus.Failed ? TestState.Failed : TestState.Error, result?.Message, stackTrace, timespan, Area, exception);
//                }
//                else
//                {
//                    return new TestResult(TestState.Error, result?.Message, stackTrace, timespan, Area, exception);
//                }
...
//    {
//        public TestResult(TestState testState, string? message, string? stackTrack, TimeSpan duration, string? area, Exception? exception)
//        {
//            Message = message;
//            TestState = testState;
